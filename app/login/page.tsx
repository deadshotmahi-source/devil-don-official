...
export default function LoginPage() {
  ...
  async function forgot() {
    if (!username) return setMessage("Enter your username first.");
    try {
      await resetPassword(username);
      setMessage("Password reset email sent to the linked auth mailbox.");
    } catch (error) {
    ...
    <main className="section grid min-h-[78vh] place-items-center">
      <form onSubmit={submit} className="glass neon-ring w-full max-w-md rounded-lg p-6">
        <div className="mb-7 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600">Access your DEVIL DON OFFICIAL dashboard.</p>
        </div>
        ...
        <div className="mt-2 flex items-center gap-2 rounded-lg border border-blue-100 bg-white/85 px-3 focus-within:border-ocean focus-within:ring-4 focus-within:ring-ocean/10">
          ...
          <input className="w-full bg-transparent py-3 text-sm outline-none" value={username} onChange={(e) => setUsername(e.target.value)} required />
        </div>
        <label className="label mt-5 block">Password</label>
        <div className="mt-2 flex items-center gap-2 rounded-lg border border-blue-100 bg-white/85 px-3 focus-within:border-ocean focus-within:ring-4 focus-within:ring-ocean/10">
          <KeyRound size={18} className="text-ocean" />
          <input className="w-full bg-transparent py-3 text-sm outline-none" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
        </div>
        ...
        <button className="btn-primary mt-6 w-full" disabled={busy}>
          {busy ? <Loader2 className="animate-spin" size={18} /> : null} Secure Login
        </button>
        <div className="mt-5 flex items-center justify-between text-sm">
          <button type="button" onClick={forgot} className="font-semibold text-ocean">Forgot password</button>
          <Link href="/register" className="font-semibold text-slate-600 hover:text-ocean">Create account</Link>
        </div>
      </form>
    </main>
  );
